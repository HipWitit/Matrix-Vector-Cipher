...
# --- 2. CSS FOR PINK BUTTONS & STYLING ---
st.markdown("""
    <style>
    .stApp { background-color: #E6E1F2 !important; }
    
    /* Targets the KISS and TELL buttons specifically */
    ...
    if det_inv:
        if kiss_btn:
            ...
            if points:
                ...
                # SAVE TO SESSION FOR COPY BUTTON
                st.session_state['last_chemistry'] = emoji_res
                st.code(emoji_res)
...
if st.button("COPY SECRET CHEMISTRY", use_container_width=True):
    if 'last_chemistry' in st.session_state:
        # This triggers a specialized text area that is easy to long-press and copy on mobile
        st.text_area("Long press to copy:", value=st.session_state['last_chemistry'], height=100)
        st.toast("Chemistry ready for copying!")
    else:
        st.warning("Create some chemistry first!")
